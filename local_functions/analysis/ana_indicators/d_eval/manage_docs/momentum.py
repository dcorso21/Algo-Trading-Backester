...
def update_momentum(current_frame):
    ...
    # ideally, these are the increments ideal for aggregation. 
...
def simplify_candles(df, start_index, end_index):
    ...
    if len(df_slice) != 0:
        ...
        dfx = dfx.rename(columns = {0:'o',1:'h',2:'l',3:'c',4:'oi',5:'hi',6:'li',7:'ci'})
...
def aggregate_df(agg_num, df):
    ...
    if (len(dfx) * agg_num) != len(df):
        left = len(df) - len(dfx) 
        dfx = dfx.append(simplify_candles(df, len(df)-left, len(df)), sort = False)
