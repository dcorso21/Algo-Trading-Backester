...
def create_second_data(df,row,mode = 'mixed'):
    if row > len(df):
        ...
        minute = list(df.iloc[row])

        o = float(minute[5])
        h = float(minute[3])
        l = float(minute[4])
        c = float(minute[2])
        v = round(float(minute[6]),1)
        ticker = minute[0]
        time = minute[1]

        prices, volumes = create_second_data_2(o,h,l,c,v,mode)
        return prices, volumes, ticker, time
        ...
        for a,b in zip(m.time, m.close):
            ...
            if a !=  minute + dt.timedelta(minutes = 1):
                ...
                for z in reps:
                    ...
                    minute = pd.to_datetime((minute + dt.timedelta(minutes = 1))).time().strftime('%H:%M:%S')
                    ...
                    minute = pd.to_datetime(minute) # -- For some reason I need this...
